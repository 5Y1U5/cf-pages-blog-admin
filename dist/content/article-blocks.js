...
const OPEN_RE = /^:::([a-z]+)[ 　]*(.*)$/;
const CLOSE_RE = /^:::[ 　]*$/;
function isBlockName(value) {
    return ARTICLE_BLOCK_NAMES.includes(value);
}
/**
 * 本文を「通常の Markdown」と「ブロック」に切り分ける。
 * 閉じ忘れたブロックは、そのまま Markdown として扱う（記事が消えるより崩れて見えるほうがよい）。
 */
export function splitArticleContent(markdown) {
...
function inline(value) {
    ...
    out = out.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, (_m, text, href) => `<a href="${href}" target="_blank" rel="noopener">${text}</a>`);
...
const CALLOUT_ICON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" ' +
 ...
 */
export function renderArticleHtml(markdown, renderMarkdown) {
    return splitArticleContent(markdown)
        .map((segment) => segment.kind === "block"
 ...
 */
export async function renderArticleHtmlAsync(markdown, renderMarkdown) {
    const parts = await Promise.all(splitArticleContent(markdown).map(async (segment) => segment.kind === "block"
        ? renderArticleBlock(segment)
