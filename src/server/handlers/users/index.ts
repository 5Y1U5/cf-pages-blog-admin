...
export function createUsersHandlers(config: BlogAdminConfig) {
  const onRequestGet: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const { results } = await db
      .prepare(
        `SELECT id, email, name, role, client_id, is_active, created_at, updated_at
         FROM users
         WHERE client_id = ?
  ...
  const onRequestPost: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const payload = await readJson<CreateUserPayload>(ctx.request);
