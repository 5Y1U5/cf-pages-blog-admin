...
interface UpdatePostPayload {
  title?: string;
  postType?: string;
  /** 省略・空・現在と同じ値なら変えない。変えると公開 URL も変わる（下記 moveGitHubPost）。 */
  slug?: string;
  date?: string;
  categorySlug?: string;
 ...
 * 未公開の下書きにはファイルが無い。
 */
function hasGitHubFile(post: Pick<PostDraftRow, "status" | "published_url" | "source_path">): boolean {
  return (
...
async function moveGitHubPost(
  ...
  fromSlug: string,
  toSlug: string
): Promise<{ ok: true; moved: boolean; tokenWarning: string | null } | Response> {
  if (fromPath === toPath) return { ok: true, moved: false, tokenWarning: null };
  const current = await readGitHubFile(env, config, fromPath);
  if (current instanceof Response) return current;
  if ("missing" in current) return { ok: true, moved: false, tokenWarning: null };
...
async function recordRedirect(
  ...
  try {
    ...
    await db
      .prepare(
        `INSERT INTO post_redirects (id, client_id, post_id, from_path, created_at)
         VALUES (?, ?, ?, ?, ?)
...
export function createPostDetailHandlers(config: BlogAdminConfig) {
  ...
  const onRequestPut: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const payload = await readJson<UpdatePostPayload>(ctx.request);
    ...
    const prior = await db
      ...
      .bind(ctx.params.id, user.client_id)
      .first<PostDraftRow>();
    if (!prior) return json({ ok: false, error: "not_found" }, { status: 404 });
    ...
    if (slugChanged && hasGitHubFile(prior)) {
      const moved = await moveGitHubPost(
        ctx.env,
        config,
        priorFilePath,
        nextFilePath,
      ...
      if (moved instanceof Response) {
        if (config.github.mode !== "backup") return moved;
    ...
    await db
      .prepare(
        "INSERT INTO revisions (id, post_id, snapshot_json, changed_by, change_note, created_at) VALUES (?, ?, ?, ?, ?, ?)"
      )
      .bind(
        randomId("rev"),
    ...
    if (slugChanged && prior.published_url && nextPublishedUrl) {
      const recorded = await recordRedirect(
        db,
        user.client_id,
        prior.id,
        prior.published_url,
      ...
      if (!recorded) {
        warning = joinWarnings(
          warning,
          "旧 URL からの転送を記録できませんでした（migration 0008 が未適用の可能性があります）。"
        );
      }
    }
  ...
  const onRequestDelete: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const post = await db
      .prepare(
        "SELECT id, slug, status, published_url, source_path FROM post_drafts WHERE id = ? AND client_id = ? LIMIT 1"
      )
      .bind(ctx.params.id, user.client_id)
      .first<{
        id: string;
    ...
    if (hasGitHubFile(post)) {
      const removed = await deleteGitHubFile(
        ctx.env,
        config,
        post.source_path || postFilePath(config, post.slug),
        `post: delete ${post.slug} from admin`
      );
      // github.mode: "backup" のサイトは公開ページが D1 を直接読むため、
      // 控えのファイルを消せなくても記事の削除自体は進める（消し残しは次の公開で上書きされる）。
      if (removed instanceof Response && config.github.mode !== "backup") return removed;
    }
