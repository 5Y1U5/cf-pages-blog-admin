...
interface CategoryUpdatePayload {
  ...
  /** 受け付けないが、送られてきたことを検出して弾くために型に置く。 */
  slug?: string;
  code?: string;
}
...
export function createCategoryDetailHandlers(config: BlogAdminConfig) {
   ...
   * 実質的な制限にならない。
   */
  const onRequestPatch: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    if (hasLabel && !label) return badRequest("表示名を入力してください。");
    ...
    if (labelChanged) {
      const result = await db
        .prepare(
          "UPDATE post_drafts SET category_label = ? WHERE client_id = ? AND category_slug = ?"
        )
