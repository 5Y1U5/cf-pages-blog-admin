...
const UPLOAD_ERROR_MESSAGES_JA: Record<string, string> = {
  "file is required.": "画像ファイルを選択してください。",
  "Only JPEG, PNG, WebP, or GIF images are allowed.":
    "アップロードできるのは JPEG・PNG・WebP・GIF の画像だけです。写真アプリなどで JPEG として保存し直してからお試しください。",
  "This file extension is not allowed.":
    "この拡張子のファイルはアップロードできません。JPEG か PNG の画像をお使いください。",
  "Image must be 5MB or smaller.":
    "画像の容量が大きすぎます。5MB 以下の画像をお使いください。",
  "File content is not a valid image.":
    "画像として読み取れませんでした。ファイルが壊れていないかご確認のうえ、別の画像をお試しください。",
  "File content does not match its declared type.":
    "画像の中身と形式が一致しませんでした。写真アプリなどで JPEG として保存し直してからお試しください。",
};
...
function formatLabel(file: File): string {
  ...
  if (/heic|heif/i.test(hint)) return "heic";
  if (/avif/i.test(hint)) return "avif";
  if (/svgz?\b|svg\+xml/i.test(hint)) return "svg";
  if (/bmp/i.test(hint)) return "bmp";
  if (/tiff?\b/i.test(hint)) return "tiff";
...
export async function prepareImageForUpload(file: File): Promise<PreparedImage> {
  ...
  if (blob.size >= file.size && scale === 1 && alreadyUploadable) return asIs();
