...
major にした（バージョニング基準 4）。**major は自動マージされない。** 導入先では
`package.json` のこのパッケージを `#v5.0.0` へ上げてデプロイするだけでよい。

- 編集画面: 既存記事を開いた直後、読み込みが終わるまで本文欄に初期のひな形（「見出しを入力…まとめ」）が
  出ていた。「本文が消えた」と見えるため、読み込み中は「記事を読み込んでいます…」だけを出すようにした。
  ひな形は新規記事だけに入れ、既存記事の本文が空のときも差し込まない
- 編集画面: 記事の読み込みに失敗したら保存・公開ボタンを押せなくし、「もう一度読み込む」を出す。
  これまでは失敗しても保存でき、画面に出ていたひな形で記事を上書きできてしまった
- 見たまま編集（通常の編集）: 装飾枠（`:::callout` `:::points` `:::compare` `:::stat` `:::faq`）を
  1つの塊として扱う。これまでは枠が段落として読み込まれ、1文字でも直して保存すると
  `:::callout 見出し 本文 :::` の1行に潰れて、公開ページで枠として描かれなくなっていた。
  塊は公開ページに近い見た目のカードで表示し、削除・並べ替えはできる。中身は「マークダウンで編集」で直す
- `ui/lib/admin-markdown` に `markdownToEditorHtml` と `articleBlockSource` を追加。
  `htmlToMarkdown` は塊を原文のまま戻す
- 見たまま編集の拡張 `ui/lib/article-block-node`（`ArticleBlock`）と、プレビュー用 CSS の
  `ui/lib/article-preview-css`（`PREVIEW_ARTICLE_CSS`）を追加
- 依存に `@tiptap/core` を明記した（pnpm の導入先で直接読めるように）。開発用に `@tiptap/html`
- テスト: 見たまま編集を通した往復で枠が崩れないことの回帰テストを追加
