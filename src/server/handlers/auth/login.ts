...
async function recentFailedAttempts(
  db: D1Database,
  email: string,
  ip: string
  ...
  try {
    ...
    console.warn(
      "login throttling is disabled: could not read login_attempts.",
...
async function recordFailedAttempt(
  ...
  try {
    ...
    console.warn(
      "login throttling is disabled: could not write login_attempts.",
      error instanceof Error ? error.message : error
    );
  }
}
...
export function createLoginHandlers(config: BlogAdminConfig) {
  const onRequestPost: PagesFunction<BlogAdminEnv> = async (ctx) => {
    const db = requireDb(ctx.env);
    if (db instanceof Response) return db;

    const payload = await readJson<LoginPayload>(ctx.request);
      ...
      return json(
        {
          ...
          message:
            "ログインの失敗が続いたため一時的にロックされています。約15分後に再度お試しください。",
        },
        { status: 429 }
      );
    }
