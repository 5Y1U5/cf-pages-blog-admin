...
export function generateInitialPassword(length = PASSWORD_LENGTH) {
    const out = [];
 ...
 * ログインもユーザー作成も通る。
 */
export async function setMustChangePassword(db, userId, value) {
    try {
        await db
        ...
        console.warn("must_change_password was not updated (migration 0006 may be missing).", error instanceof Error ? error.message : error);
    }
}
export async function readMustChangePassword(db, userId) {
    try {
        const row = await db
