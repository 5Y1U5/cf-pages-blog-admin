...
const REQUIREMENT_LABELS: Record<PublishRequirement, string> = {
  title: "タイトル",
  body: "本文",
  slug: "slug",
  date: "公開日",
...
function firstImageFromBody(markdown: string): string | null {
  const match = markdown.match(/!\[[^\]]*\]\(([^)]+)\)/);
  return match ? match[1].trim() : null;
}

function hasRequirement(post: PostDraftRow, field: PublishRequirement): boolean {
  switch (field) {
    case "title":
...
async function resetPublishingStatus(
  db: D1Database,
  post: PostDraftRow,
  userId: string
): Promise<void> {
...
export function createPublishHandlers(config: BlogAdminConfig) {
  const onRequestPost: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const preferredCategory = resolveDefaultCategory<CategoryRow>(
      config,
      categoryRows.filter((category) => category.is_active === 1)
    );
    const categorySlug =
      post.category_slug || preferredCategory?.slug || config.category.defaultSlug;
    const categoryLabel =
      post.category_label || preferredCategory?.label || config.category.defaultLabel;
    ...
    const effectivePost: PostDraftRow = {
      ...post,
      date,
    ...
    const postCommit = await upsertGitHubFile(
      ctx.env,
      config,
      postFilePath(config, effectivePost.slug),
      markdown,
