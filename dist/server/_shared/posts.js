...
function categoryCodeForDraft(draft, categories) {
    return (categories.find((category) => category.slug === draft.category_slug || category.code === draft.category_slug)?.code ||
        draft.category_slug);
}
export function draftToMarkdown(draft, config, categories = []) {
    ...
    const heroImage = draft.hero_image_key && draft.hero_image_key.startsWith("/")
