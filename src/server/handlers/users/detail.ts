...
export function createUserDetailHandlers(config: BlogAdminConfig) {
   ...
   */
  const changeOwnPassword: PagesFunction<BlogAdminEnv> = async (ctx) => {
    ...
    const payload = await readJson<ChangePasswordPayload>(ctx.request);
  ...
  const onRequestPut: PagesFunction<BlogAdminEnv> = async (ctx) => {
    if (String(ctx.params.id) === SELF_ID) return changeOwnPassword(ctx);
