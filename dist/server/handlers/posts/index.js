...
async function ensureUniqueSlug(db, clientId, base) {
...
export function createPostsHandlers(config) {
    const onRequestGet = async (ctx) => {
        ...
        const { results } = await db
            .prepare(`SELECT id, slug, title, date, category_slug, category_label, status,
    ...
    const onRequestPost = async (ctx) => {
        ...
        await db
            .prepare(`INSERT INTO post_drafts
