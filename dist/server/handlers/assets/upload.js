...
export function createAssetUploadHandlers(config) {
    const onRequestPost = async (ctx) => {
        ...
        if (db instanceof Response)
            return db;
        if (!ctx.env.ADMIN_ASSETS)
            return serverError("ADMIN_ASSETS binding is not configured.");
        ...
        if (hasDeniedExtension(upload.name))
            return badRequest("This file extension is not allowed.");
