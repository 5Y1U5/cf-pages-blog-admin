...
| 操作 | API | 権限 |
|---|---|---|
| 一覧 | `GET /api/admin/categories` | 全員 |
| 追加 | `POST /api/admin/categories` | `admin` / `client_publisher` |
| 表示名・説明の変更 | `PATCH /api/admin/categories/<id>` | `admin` / `client_publisher` |
| 削除 | `DELETE /api/admin/categories/<id>` | `admin` |

- **`slug` と `code` は変更できない。** 記事は `category_slug` で紐づき、書き出した Markdown の
  frontmatter も `code` を持っているため、変えると既存記事との対応が切れる。
  送ると 400 で断る。URL を変えたい場合は作り直して記事を移す
- 表示名を変えると、そのカテゴリの記事（`post_drafts.category_label`）も同時に追随する。
  片方だけ直すと、一覧と記事ページで名前が食い違う
- 改名すると、そのカテゴリの**サイトに出ている記事の Markdown も同時に書き戻す**。
  公開時に frontmatter へ表示名を焼き込んでいる（`categoryLabel`）ため、ここを直さないと
  一覧と記事ページで名前が食い違う。対象は `status = 'published'` の記事に加えて、
  公開中の記事を開いて保存したもの（`status` は `draft` に戻るが `published_url` は残る）も含む。
  後者は `draftToMarkdown` で組み立て直さず、いまのファイルの `categoryLabel` の行だけ差し替える
  （組み立て直すと、まだ公開していない編集や `draft: true` まで書き出してしまう）。
  読めなかった記事は書き換えずに応答の `skippedPosts` で返す。
  公開取り下げ済みの記事は `published_url` が NULL なので対象外
  （ファイル自体は `draft: true` で残っている。取り下げでは消していない）
- カテゴリ一覧の JSON と記事の Markdown は **Git Data API で1コミットにまとめて書く**。
  途中で失敗して「一部の記事だけ新しい名前」という状態を残さないため。
  GitHub への呼び出しは記事数によらず5回
- 削除は使っている記事が1本でもあれば断る（`is_active = 0` にする論理削除）
- JSON の書き出しに失敗した場合は D1 の変更も元へ戻す。ずれたまま残すと、
  次の公開で古い名前へ黙って戻ってしまうため
...
ユーザーの追加／変更／削除／パスワード再発行・カテゴリの追加／変更／削除・画像アップロードを記録する。
