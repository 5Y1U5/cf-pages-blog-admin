...
export function createCategoriesHandlers(config) {
    ...
    const onRequestPost = async (ctx) => {
        ...
        const categories = await db
            ...
            .bind(user.client_id)
            .all();
        const commit = await upsertGitHubFile(ctx.env, config, config.content.categoriesJsonPath, categoryRowsToJson(categories.results || []), `chore: add blog category ${slug} from admin`);
        if (commit instanceof Response) {
            if (previous) {
                await db
                    .prepare(`UPDATE categories SET label = ?, description = ?, is_active = ?, updated_at = ?
             WHERE id = ? AND client_id = ?`)
                    .bind(previous.label, previous.description, previous.is_active, nowIso(), row.id, user.client_id)
                    .run();
            }
            else {
                await db
