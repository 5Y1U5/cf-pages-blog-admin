...
export function createCategoryDetailHandlers(config) {
    const onRequestDelete = async (ctx) => {
        ...
        const activeCategories = await db
            ...
            .bind(user.client_id)
            .all();
        const categoryCommit = await upsertGitHubFile(ctx.env, config, config.content.categoriesJsonPath, categoryRowsToJson(activeCategories.results || []), `chore: remove blog category ${category.slug} from admin`);
