...
export function AdminEditorClient({ config, router }: AdminEditorClientProps) {
  ...
  useEffect(() => {
    if (hasMissingInAdvanced) setShowAdvanced(true);
  ...
  async function unpublish() {
    ...
    if (!res.ok) {
      setMessage(
        readError(data, "公開を取り下げできませんでした。時間をおいて再度お試しください。")
      );
      return;
    }
    setStatus("draft");
        ...
        <div className="mx-auto flex max-w-[1180px] items-center justify-between gap-3">
          ...
          <div className="hidden gap-2 md:flex">
            <button
              ...
              disabled={isSaving || isLoading || isUnpublishing}
            ...
            <button
              ...
              disabled={!canPublish || isPublishing || isSaving || isUnpublishing}
              title={
                canPublish ? "公開する" : `公開には次の入力が必要です：${missingFields.join("・")}`
              }
              ...
              <section className="rounded-lg border border-border bg-background p-4">
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  <button
                    ...
                    className="flex h-11 items-center justify-center gap-2 rounded-lg border border-border text-[13px] font-bold"
                  >
                    <Plus size={16} />
                    追加
                  </button>
